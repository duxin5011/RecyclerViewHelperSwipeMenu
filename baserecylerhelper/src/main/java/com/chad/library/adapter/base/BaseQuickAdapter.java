...
import com.chad.library.adapter.base.swipemenu.SwipeItemLayout;
import com.chad.library.adapter.base.swipemenu.SwipeMenu;
import com.chad.library.adapter.base.swipemenu.SwipeMenuCreator;
import com.chad.library.adapter.base.swipemenu.SwipeMenuLayout;
...
public abstract class BaseQuickAdapter<T, K extends BaseViewHolder> extends RecyclerView.Adapter<K> {
    ...
    private OnItemSwipeMenuListener onItemSwipeMenuListener;
    ...
    public K onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            ...
            default:
                ...
                bindSwipeMenuListener(baseViewHolder);
        }
     ...
     * @
     */
    private void bindSwipeMenuListener(final BaseViewHolder baseViewHolder) {
        if (baseViewHolder == null || swipeMenuCreator == null) {
            return;
        }
        final View itemView = baseViewHolder.itemView;
        if (itemView != null && itemView instanceof SwipeItemLayout) {
            final SwipeItemLayout swipeItemLayout = (SwipeItemLayout) itemView;
            SwipeMenuLayout swipeMenuLayout = baseViewHolder.getView(R.id.swipeMenuLayout);
            SwipeMenu swipeMenu = new SwipeMenu(mContext);
            swipeMenuCreator.SwipeMenuCreat(swipeMenu);
            swipeMenuLayout.setSwipeMenu(swipeMenu);
            swipeItemLayout.addSwipeView(swipeMenuLayout);
            if (getOnItemSwipeMenuListener() != null) {
                ArrayList<LinearLayout> linearLayouts = swipeMenuLayout.getLinearLayouts();
                for (final LinearLayout linearLayout : linearLayouts) {
                    linearLayout.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            getOnItemSwipeMenuListener().onItemSwipeMenu(baseViewHolder.getLayoutPosition() - getHeaderLayoutCount(), linearLayout.getId());
    ...
    private void isEnableSwipe(K holder) {
        boolean enable = isEnableSwipeMenu(holder.getLayoutPosition() - getHeaderLayoutCount());
        ((SwipeItemLayout) holder.itemView).setmSwipeEnable(enable);
    }
    ...
    protected K onCreateDefViewHolder(ViewGroup parent, int viewType) {
        ...
        SwipeItemLayout swipeMenuLayout = (SwipeItemLayout) getItemView(R.layout.recycler_view_swipe_menu, parent);
        View itemView = getItemView(layoutId, parent);
    ...
    /**
     * @author duxin
     *         Add the sideslip menu click on listening
     */

    public interface OnItemSwipeMenuListener {

        void onItemSwipeMenu(int position, int index);

    }

    //// TODO: 2017/6/9  duxin
    public void setOnItemSwipeMenuListener(@NonNull OnItemSwipeMenuListener onItemSwipeMenuListener) {
        this.onItemSwipeMenuListener = onItemSwipeMenuListener;
    }

    public OnItemSwipeMenuListener getOnItemSwipeMenuListener() {
        return this.onItemSwipeMenuListener;
    }
